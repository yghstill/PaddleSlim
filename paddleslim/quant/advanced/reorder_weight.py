...
from paddleslim.quant.layers import ShiftScalingRowParallelLinear
...
class ReorderFFNWeight():
    ...
    def _sample_absmax(self, input, layer):
        ...
        if type(layer) == ShiftScalingRowParallelLinear:
            x = paddle.add(x, layer.shift_weight)
    ...
    def _apply_hook(self):
        ...
        for layer_name, sub_layer in self.model.named_sublayers():
            if type(sub_layer) in [
                    RowParallelLinear, ShiftScalingRowParallelLinear
